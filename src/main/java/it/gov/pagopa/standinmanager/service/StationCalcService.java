...
public class StationCalcService {
  ...
  private void removeStationManually(List<CosmosStandInStation> stations) {
    ...
    String emailBody =
        String.format(
            "[StandInManager] Station [%s] has been removed from stand-in manually",
  ...
  private void removeStation(
      List<CosmosStandInStation> stations, String eventInfo, String emailBody) {
    log.info(eventInfo);
    ...
    cosmosEventsRepository.newEvent(station, Constants.EVENT_REMOVE_FROM_STANDIN, eventInfo);

    String sendResult =
        awsSesClient.sendEmail(
            String.format("[StandInManager][%s] Station [%s] removed from stand-in", env, station),
            emailBody);
