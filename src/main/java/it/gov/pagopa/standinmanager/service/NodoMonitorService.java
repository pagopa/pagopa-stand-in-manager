...
public class NodoMonitorService {
  ...
  /**
   * Monitor traffic for each station that is eligible for Stand-In. Retrieves station total call
   * and fault in the configured slot time {@link NodoMonitorService#slotMinutes} and retrieve the
   * total call in the configured slot. Saves the counts on CosmosDB.
   *
  ...
  public void getAndSaveData() throws DataServiceException, DataClientException {
    ...
    Map<String, Integer> totalCallForStation =
        getCountGroupedByStations(
            TOTAL_CALL_GROUP_BY_STATION_QUERY, includedOrExcludedStations, timeLimit);

    Set<String> stationsToInclude = totalCallForStation.keySet();
    Map<String, Integer> faultForStation =
        getCountGroupedByStations(
            FAULT_CALL_GROUP_BY_STATION_QUERY,
            new IncludedOrExcludedStations(stationsToInclude),
            timeLimit);
    ...
    if (log.isDebugEnabled()) {
      ...
      totalCallForStation.forEach(
          (key, value) ->
              totalsString.append(
                  String.format(
                      "%n%s calls: %s faults: %s", key, value, faultForStation.get(key))));
  ...
  private Map<String, Integer> getCountGroupedByStations(
      String query, IncludedOrExcludedStations includedOrExcludedStations, ZonedDateTime timeLimit)
    ...
    if (exclusionList != null && !exclusionList.isEmpty()) {
      String stations =
          exclusionList.stream().map(s -> "'" + s + "'").collect(Collectors.joining(","));
      replacedQuery =
          query.replace(STATIONS_FILTER_PLACEHOLDER, "| where stazione !in(" + stations + ")\n");
    } else if (inclusionList != null && !inclusionList.isEmpty()) {
      String stations =
          inclusionList.stream().map(s -> "'" + s + "'").collect(Collectors.joining(","));
      replacedQuery =
          query.replace(STATIONS_FILTER_PLACEHOLDER, "| where stazione in (" + stations + ")\n");
    } else {
  ...
  private Integer getTotalCallCount(ZonedDateTime timeLimit)
      throws DataServiceException, DataClientException {
    log.debug("Running KQL query [{}]", TOTAL_CALL_QUERY);
    KustoOperationResult response =
        this.kustoClient.execute(this.database, TOTAL_CALL_QUERY, getTimeParameters(timeLimit));
    KustoResultSetTable primaryResults = response.getPrimaryResults();
  ...
  private record IncludedOrExcludedStations(Set<String> inclusionList, Set<String> exclusionList) {
