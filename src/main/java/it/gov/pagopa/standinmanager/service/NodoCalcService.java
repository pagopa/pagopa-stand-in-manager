...
public class NodoCalcService {
  ...
  /**
   * Elaborate the monitored stations traffic in the configured range time {@link
   * NodoCalcService#rangeMinutes} aggregated by the configured slot time {@link
   * NodoCalcService#slotMinutes} and activates the Stand-In mode if all the following condition
   * occur:
   *
   * <ul>
   *   <li>During nighttime hours, from {@link NodoCalcService#nighttimeStartHours}:{@link
   *       NodoCalcService#nighttimeStartMinutes} to {@link
   *       NodoCalcService#nighttimeEndHours}:{@link NodoCalcService#nighttimeEndMinutes}
   *       <ul>
   *         <li>Stations have an availability below the configured threshold {@link
   *             NodoCalcService#rangeThreshold}
   *         <li>The total payment traffic managed by the station exceeds the configured threshold
   *             {@link NodoCalcService#totalTrafficNightThreshold} of the total traffic of NodoSPC
   *       </ul>
   *   <li>During daytime hours, from {@link NodoCalcService#nighttimeEndHours}:{@link
   *       NodoCalcService#nighttimeEndMinutes} to {@link
   *       NodoCalcService#nighttimeStartHours}:{@link NodoCalcService#nighttimeStartMinutes}
   *       <ul>
   *         <li>Stations have an availability below the configured threshold {@link
   *             NodoCalcService#rangeThreshold}
   *         <li>The total payment traffic managed by the station exceeds the configured threshold
   *             {@link NodoCalcService#totalTrafficDayThreshold} of the total traffic of NodoSPC
   *       </ul>
   */
  public void runCalculations() {
    ...
    allStationCounts.forEach(
        (station, stationCounts) -> {
          ...
          long numOfHighTrafficSlots =
              filteredCallCounts.stream()
                  .filter(c -> c.getTotalTrafficPercentage() > totalTrafficThreshold)
                  .count();
          long numOfFailedSlots =
              filteredCallCounts.stream()
                  .filter(c -> c.getFaultPercentage() > this.slotThreshold)
                  .count();
  ...
  private @NotNull List<CosmosNodeCallCounts> filterDuplicatedSlotData(
      List<CosmosNodeCallCounts> stationCounts) {
    Map<Instant, List<CosmosNodeCallCounts>> groupNodeCallCountsToSlotInterval =
    ...
    return groupNodeCallCountsToSlotInterval.values().stream()
        .map(
            cosmosNodeCallCounts ->
                cosmosNodeCallCounts.stream()
                    .sorted(
                        Comparator.comparing(
                            s -> s.getTimestamp().getEpochSecond(), Comparator.reverseOrder()))
                    .toList()
                    .get(0))
        .toList();
  }
  ...
  private Instant roundToNearestSlotMinutes(Instant instant) {
    ...
    long roundedSeconds =
        Math.floorDiv(epochSeconds, this.slotMinutes * 60) * this.slotMinutes * 60;
