...
//  static {
//    try {
//      kustoEmulator = new GenericContainer(DockerImageName.parse("mcr.microsoft.com/azuredataexplorer/kustainer-linux:latest"))
...
//  }
  ...
  private static void startTcpProxy(Integer... ports) {
    for (Integer port : ports) {
      StaticTcpProxyConfig tcpProxyConfig =
          new StaticTcpProxyConfig(port, cosmosEmulator.getHost(), cosmosEmulator.getMappedPort(port));
  ...
  public void initialize(ConfigurableApplicationContext applicationContext) {
    try {
      ...
      keyStore.store(
          new FileOutputStream(keyStoreFile.toFile()), cosmosEmulator.getEmulatorKey().toCharArray());
